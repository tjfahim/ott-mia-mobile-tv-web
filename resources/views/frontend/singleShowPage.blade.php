...
<section class="bg-cover relative w-full h-[650px]" style="background-image:  linear-gradient(0deg, rgba(20,20,20,0.4878151944371498) 0%, rgba(20,20,20,0.20770314961922265) 100%), url({{ URL::to( 'upload/source/'.$show->video_image_thumb )}})">
    ...
    <div class="absolute  bottom-0 left-0 right-0 text-white text-center p-10 space-y-5">
        <div class="flex flex-col gap-2">
            <h2 class="text-xl font-semibold">{{ $show->series_name }}</h2>
            {{-- <p class="opacity-50">{{ $show->video_description }}</p> --}}
        </div>
        <div class="flex justify-center items-stretch gap-3">
            <a href="{{  URL::to('show/play/'. $show->series_slug) }}" class="bg-redcolor flex rounded gap-1 px-3 py-2 justify-center items-center text-md "><img src="{{ URL::asset('assets/frontend/images/play-Icon.svg') }}" alt=""><span>Play Now</span></a>
            ...
            <button class=" border-2 border-[#262626]  p-2 px-3 bg-[#0F0F0F] rounded"><img  class="size-8" src="{{  URL::asset('assets/frontend/images/like.svg') }}" alt=""></button>
