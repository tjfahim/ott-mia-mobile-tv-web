...
<header id="header" class="header">
  <nav id="navbar" class="navbar">
    ...
    <a href="{{ URL::to('/') }}" class="logo">
      <img src="{{ URL::asset('upload/source/'.getcong('site_logo')) }}" alt="Brand Logo" width="76" class="brand__logo" />
    </a>
    ...
    <ul class="navbar__nav">
      ...
      <li class="nav__item dropdown">
        <a href="{{ URL::to('series') }}" class="nav__link dropdown__trigger">{{trans('words.tv_shows_text')}}</a>
        <ul class="dropdown__menu">
          <li class="dropdown__item">
            <a href="{{ URL::to('language/series') }}" class="dropdown__link">{{trans('words.language_text')}}</a>
          </li>
          <li class="dropdown__item">
            <a href="{{ URL::to('genres/series') }}" class="dropdown__link">{{trans('words.genres_text')}}</a>
          </li>
        </ul>
      </li>
      <li class="nav__item ">
        <a href="{{ route('reels')}}" class="nav__link">{{trans('words.reals')}}</a>

      </li>
      <li class="nav__item dropdown">
        <a href="{{ URL::to('movies') }}" class="nav__link dropdown__trigger">{{trans('words.movies_text')}}</a>
        <ul class="dropdown__menu">
          <li class="dropdown__item">
            <a href="{{ URL::to('language/movies') }}" class="dropdown__link">{{trans('words.language_text')}}</a>
          </li>
          <li class="dropdown__item">
            <a href="{{ URL::to('genres/movies') }}" class="dropdown__link">{{trans('words.genres_text')}}</a>
          </li>
        </ul>
      </li>

      <li class="nav__item dropdown">
        <a href="{{ URL::to('sports') }}" class="nav__link dropdown__trigger">{{trans('words.sports_text')}}</a>
        <ul class="dropdown__menu">
          @foreach(\App\SportsCategory::where('status','1')->orderBy('category_name')->get() as $sports_cat)
          <li class="dropdown__item">
            <a href="{{ URL::to('sports/'.$sports_cat->category_slug) }}" class="dropdown__link">{{$sports_cat->category_name}}</a>
          </li>
          @endforeach
        </ul>
      </li>
      ...
      <li class="nav__item dropdown">
        <a href="{{ URL::to('live-tv') }}" class="nav__link dropdown__trigger">{{trans('words.live_tv')}}</a>
        <ul class="dropdown__menu">
          @foreach(\App\TvCategory::where('status','1')->orderBy('category_name')->get() as $tv_cat)
          <li class="dropdown__item">
            <a href="{{ URL::to('live-tv/'.$tv_cat->category_slug) }}" class="dropdown__link">{{$tv_cat->category_name}}</a>
          </li>
          @endforeach
        </ul>
      </li>
    ...
    <ul class="navbar__nav ms-auto d-none d-xl-flex">
      ...
      <!--    <img src="{{ URL::asset('images/en__bn__translate.png')}}" alt="translate icon" width="24" />-->
      ...
      <li class="nav__item dropdown">
        <button type="button" class="translate__btn">
          ...
          <img src="{{ URL::asset('upload/'.Auth::User()->user_image) }}" alt="profile_img" style="height: 25px;width: 25px;">
          @else
          <img src="{{ URL::asset('site_assets/images/avatar.jpg') }}" alt="profile_img" style="height: 25px;width: 25px;">
          @endif
          <span class="translate__btn__text ms-1"> Hi, {{ Str::limit(Auth::User()->name,6)}}</span>
          <i class="fa-solid fa-caret-down"></i>
        </button>
        <ul class="dropdown__menu__sm">
          <li class="dropdown__item">
            <a href="{{ URL::to('dashboard') }}" class="dropdown__link">{{trans('words.dashboard_text')}}</a>
          </li>
          <li class="dropdown__item">
            <a href="{{ URL::to('profile') }}" class="dropdown__link">{{trans('words.profile')}}</a>
          </li>
          <li class="dropdown__item">
            <a href="{{ URL::to('logout') }}" class="dropdown__link">{{trans('words.logout')}}</a>
          </li>
        </ul>
      </li>
      @else
      ...
      <li class="nav__item">
        <a href="{{ URL::to('login') }}" style="text-transform: uppercase;"> <button type="button" class="nav__btn login__btn">
...
<header id="header__sm" class="header__sm">
  <nav id="navbar__sm" class="navbar__sm">
    ...
    <ul class="navbar__nav navbar__nav__bottom">
      ...
      <li class="nav__item">
        <a href="{{ URL::to('login') }}" style="text-transform: uppercase;">
  ...
  <div class="filter__box">
    ...
    {!! Form::open(array('url' => 'search','class'=>'mb-5','id'=>'search','role'=>'form','method'=>'get')) !!}
    <div class="input-group">
      <input type="text" name="s" class="form-control" placeholder="{{trans('words.search')}}" aria-label="Search field" aria-describedby="search__field" id="search__field" />
