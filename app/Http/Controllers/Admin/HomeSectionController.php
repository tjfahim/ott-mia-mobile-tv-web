...
class HomeSectionController extends MainAdminController
{
    ...
    public function home_section()
    {
        ...
        $live_list = LiveTV::orderBy('id','DESC')->get();
        $series_list = Series::orderBy('id','DESC')->get();

        $ivtv_categories = IptvCategorie::orderBy('id','DESC')->get();

        return view('admin.pages.home_section',compact('ivtv_categories', 'page_title','language_list','movies_list','series_list','home_settings', 'live_list'));
    }
    ...
    public function update_home_section(Request $request)
    {
        ...
        if(isset($inputs['ivtv_movie_cat']))
        {
        $settings->movies_categories = implode(',', $inputs['ivtv_movie_cat']);
        }

        if(isset($inputs['ivtv_shows_cat']))
        {
        $settings->shows_categories = implode(',', $inputs['ivtv_shows_cat']);
        }
        ...
        if(isset($inputs['section_live_now']))
        {
        $settings->section_live_now = implode(',', $inputs['section_live_now']);
        }
        ...
        if(isset($inputs['section3_tranding_movie']))
        {
        $settings->section3_tranding_movie = implode(',', $inputs['section3_tranding_movie']);
        }
